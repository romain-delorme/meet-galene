...
function useToggleCamera(){
    const [cameraEnabled, setCameraEnabled] = useState(false);

    const [cameraIconPath, setCameraIconPath] = useState(STYLE_PATH + 'assets/icons/camera-icon.svg')

    useEffect(function(this: ServerConnection) {
        setCameraIconPath(cameraEnabled? STYLE_PATH + 'assets/icons/camera-disabled-icon.svg' : STYLE_PATH + 'assets/icons/camera-icon.svg');
